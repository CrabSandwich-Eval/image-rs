...
if [ "$fuzzer" = "libfuzzer" ] || [ "$fuzzer" = "libafl_libfuzzer" ]; then
  taskset -c "$core" timeout -s SIGKILL "$time" "./$BENCHMARK" -fork=1 -ignore_ooms=1 -ignore_timeouts=1 -ignore_crashes=1 -detect_leaks=0 -artifact_prefix=./artifacts/ ./output ./seeds
  llvm_cov="/root/.rustup/toolchains/nightly-2023-08-14-x86_64-unknown-linux-gnu/lib/rustlib/x86_64-unknown-linux-gnu/bin/llvm-cov"
	...
	# image-rs needs longer timeout
  taskset -c "$core" timeout -s SIGKILL "$time" "./$BENCHMARK" --cores all --output ./output --input ./seeds --timeout 10000
